...
export class FichedepaieComponent implements OnInit {
  ...
  listDemandes: DemandeFicheDePaie[];
  settings = {
    columns: {
      requestDate: {
        title: 'Date',
  ...
  fetchDemandes(id?: any) {
    if (id) {
      this.demandeFicheDePaieService
        .findDemandeFicheDePaieForChefDepartement(id)
        .subscribe((res) => {
      ...
      this.demandeFicheDePaieService
        .findAllRecentDemandesFicheDePaieForAdmin()
        .subscribe((res) => {
          this.listDemandes = res;
          this.source.load(this.listDemandes);
        });
  ...
  goToDemandePage(demande: any) {
    let demandeType = 'demande-fiche-de-paie';
